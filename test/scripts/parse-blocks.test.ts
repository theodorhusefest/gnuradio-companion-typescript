...
import { findBlockFiles, getBlockCategory, parseBlockFile } from '../../scripts/parse-blocks'
...
describe('parse-blocks', () => {
  ...
  describe('parseBlockFile', () => {
    ...
    it('should return null for invalid block file (missing required fields)', () => {
      const invalidBlockPath = path.join(fixturesPath, 'invalid_block.block.yml')
    ...
    it('should parse nested block file', () => {
      const nestedBlockPath = path.join(fixturesPath, 'subfolder', 'nested_block.block.yml')
  ...
  describe('findBlockFiles', () => {
    it('should find all .block.yml files in directory', () => {
      ...
      expect(files.some((f) => f.endsWith('invalid_block.block.yml'))).toBe(true)
      expect(files.some((f) => f.endsWith('nested_block.block.yml'))).toBe(true)
    })
