 ...
 */
export function getPorts(ports: BlockPort[] | undefined, shouldShowPorts: boolean): BlockPort[] {
  return ports?.filter((port) => !port.optional || shouldShowPorts) ?? []
}

export const getPortColor = (port: BlockPort, blockDType: string | undefined): string => {
...
export function getPortDTypeFromNode(
  ...
  const ports =
    portType === 'input' ? node.data.blockDefinition.inputs : node.data.blockDefinition.outputs
